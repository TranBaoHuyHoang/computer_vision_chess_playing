...
# image1 = cv2.imread('captured_image3.jpg')
...
imagedrawed = cv2.drawContours(image, [biggest_cnt], -1, random_color(), 2)

transformed = perspective_transform(image, biggest_cnt)
# transformed1 = perspective_transform(image1, biggest_cnt)

transformed = rotate_image(transformed, 90)
# transformed1 = rotate_image(transformed1, 90)
...
# dst1 = cv2.warpPerspective(transformed1, M, OUTPUT_IMAGE_SIZE)

# #Get the absolute difference between the initial and final frames.
# if dst.shape != dst1.shape:
#     # Resize image2 to match the shape of image1
#     dst1 = cv2.resize(dst1, (dst.shape[1], dst.shape[0]))

# diff = cv2.absdiff(dst, dst1)

# retval, mask_thresh = cv2.threshold(diff, 35, 255, cv2.THRESH_BINARY)
...
cv2.imshow('Webcam1', transformed)
